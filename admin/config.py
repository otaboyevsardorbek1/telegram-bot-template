...
ADMIN_HOST: str = os.getenv("ADMIN_HOST") or "0.0.0.0"
...
BABEL_DEFAULT_LOCALE = os.getenv("BABEL_DEFAULT_LOCALE") or "uz"
...
def database_url() -> str:
    ...
    # Agar barcha kerakli ma'lumotlar mavjud bo'lsa, PostgreSQL'ga ulanadi
    if db_host and db_port and db_user and db_name:
        db_port = int(db_port)  # Portni int ga o‘girish
        if db_pass:
            return f"postgresql://{db_user}:{db_pass}@{db_host}:{db_port}/{db_name}"
        return f"postgresql://{db_user}@{db_host}:{db_port}/{db_name}"

    # Agar ma'lumotlar mavjud bo'lmasa, SQLite3'ga ulanadi
    return "sqlite:///offline_database.db"
